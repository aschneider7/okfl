...
export function AppShell({children}: {children: React.ReactNode}) {
  ...
  useEffect(()=>{document.documentElement.classList.toggle("nav-open",open);return()=>document.documentElement.classList.remove("nav-open")},[open]);
  ...
  return <div className="shell">
    ...
    <nav className="mobileDock" aria-label="Primary mobile navigation">{mobileLinks.map(([href, label, glyph]) => { const active = path === href || (href !== "/" && path.startsWith(href)); const mobileLabel=label === "League Calendar" ? "Calendar" : label === "Live Draft" ? "Draft" : label === "Live Dashboard" ? "Live" : label === "Weekly Recap" ? "Recap" : label === "Playoff Odds" ? "Odds" : label; return <Link key={href} href={href} onClick={()=>beginNavigation(href)} className={active ? "active" : ""} aria-current={active?"page":undefined}><i>{glyph}</i><span>{mobileLabel}</span></Link>; })}</nav>
