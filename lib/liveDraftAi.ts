...
export function chooseLiveDraftAiPlayer(snapshot: LiveDraftSnapshot, currentOverall: number, pool:DraftPlayer[]=fallbackPprPool()): DraftPlayer | null {
  ...
  const available = pool.filter((player) => !drafted.has(draftPlayerKey(player.name)));
