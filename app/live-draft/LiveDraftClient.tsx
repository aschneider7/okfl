...
export function LiveDraftClient({initialCode}: {initialCode: string}) {
  ...
  const canPick = Boolean(snapshot?.room.status === "live" && account?.franchiseId===credentials.franchiseId && currentSeat?.franchiseId === credentials.franchiseId && credentials.seatToken);
  ...
  return <div className="draftRoomV3 liveDraftRoom">
    ...
    <section className="liveSeatLobby"><header><div><span className="eyebrow">Draft lobby</span><h2>{snapshot.seats.filter((seat) => seat.claimed).length}/10 franchises claimed</h2></div>{isHost && <div className="liveHostControls">
      ...
      return <article key={seat.franchiseId} className={`${seat.claimed ? "claimed" : ""} ${mine ? "mine" : ""} ${aiControlled ? "ai" : ""}`}><i>{aiControlled ? "AI" : seat.managerName.slice(0, 1)}</i><span>Pick {seat.slot}</span><b>{seat.managerName}</b><small>{seat.claimedName || (aiControlled ? "AI manager" : "Awaiting manager")}</small><em>{onlineFranchises.has(seat.franchiseId) ? "Online" : seat.claimed ? "Authenticated · offline" : aiControlled ? "AI controlled" : "Waiting for sign-in"}</em></article>;
    })}</div>
