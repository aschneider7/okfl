...
export function LiveDraftClient({initialCode}: {initialCode: string}) {
  ...
  async function createRoom(event: React.FormEvent) {
    ...
    try {
      const result = await requestJson("/api/live-draft/rooms", {method: "POST", body: JSON.stringify({roomName, clockSeconds})},authFetch as typeof fetch);
      ...
      window.history.replaceState(null, "", `/live-draft?room=${result.snapshot.room.code}`); setMessage("Official room created. Signed-in managers claim their own seats automatically.");
  ...
  if (!snapshot) return <section className="liveDraftLaunch">
    <div className="liveLaunchIntro"><span className="liveTag">Account-secured multiplayer</span><h2>One board. Ten verified franchises.</h2><p>A Commissioner creates the official room after locking keepers. Every signed-in manager automatically receives the correct franchise.</p><div className="liveFeatureStrip"><span>10 live seats</span><span>No team PINs</span><span>Custom pick clock</span><span>Automatic AI teams</span></div></div>
    <div className="liveLaunchForms">
      {account?.role==="commissioner"&&!account.mustChangePassword?<form onSubmit={createRoom}><span>Commissioner · {account.displayName}</span><h3>Create the official room</h3><label>Room name<input value={roomName} onChange={(event) => setRoomName(event.target.value)} /></label><fieldset className="clockSetup"><legend>Seconds per pick</legend><div>{[10,30,60].map((value) => <button type="button" key={value} className={clockSeconds === value ? "active" : ""} onClick={() => setClockSeconds(value)}>{value === 60 ? "1 min" : `${value}s`}</button>)}<label>Custom<input aria-label="Custom seconds per pick" required type="number" min="1" max="3600" value={clockSeconds} onChange={(event) => setClockSeconds(Math.max(1, Math.min(3600, Number(event.target.value) || 1)))} /><span>sec</span></label></div></fieldset><button disabled={busy}>Create room from locked keepers</button></form>:<div className="liveAuthNotice"><span>Commissioner</span><h3>Official room creation</h3><p>{authLoading?"Checking account access…":"A verified Commissioner account is required to create the official live room."}</p>{!account&&<Link href="/login?next=/live-draft">Sign in</Link>}{account&&account.role!=="commissioner"&&<small>Signed in as {account.displayName} · {account.franchiseName}</small>}</div>}
