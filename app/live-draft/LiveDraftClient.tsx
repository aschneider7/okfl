...
type JsonResult = {snapshot?: LiveDraftSnapshot; error?: string; hostToken?: string; seatToken?: string; franchiseId?: string; displayName?: string};

async function requestJson(url: string, options?: RequestInit, fetcher:typeof fetch=fetch): Promise<JsonResult> {
  const response = await fetcher(url, {...options, headers: {"Content-Type": "application/json", ...(options?.headers || {})}});
...
export function LiveDraftClient({initialCode}: {initialCode: string}) {
  ...
  const [keeperSource, setKeeperSource] = useState<"projected"|"official">("projected");
  const [secondsRemaining, setSecondsRemaining] = useState(30);
  const channelRef = useRef<RealtimeChannel | null>(null);
  const serverOffsetRef = useRef(0);
  const automatedOverallRef = useRef(0);
  const timeoutOverallRef = useRef(0);
  const joiningAccountRef=useRef("");
  ...
  async function createRoom(event: React.FormEvent) {
    ...
    try {
      const result = await requestJson("/api/live-draft/rooms", {method: "POST", body: JSON.stringify({roomName, clockSeconds, keeperSource})},authFetch as typeof fetch);
      if (!result.snapshot || !result.hostToken) throw new Error("The room response was incomplete.");
      const next = {roomCode: result.snapshot.room.code, hostToken: result.hostToken,seatToken:result.seatToken,franchiseId:result.franchiseId,displayName:result.displayName};
      saveCredentials(next); setSnapshot(result.snapshot); setRoomCodeInput(result.snapshot.room.code);
      window.history.replaceState(null, "", `/live-draft?room=${result.snapshot.room.code}`); setMessage(`${keeperSource==="projected"?"Projected Mock Draft":"Locked official"} keepers loaded. Signed-in managers claim their own seats automatically.`);
  ...
  async function rejoinDraft(){
    if(!snapshot||!account)return;
    setBusy(true);setMessage(`Returning ${account.franchiseName} to manual drafting…`);
    try{
      const result=await requestJson(`/api/live-draft/rooms/${snapshot.room.code}/join`,{method:"POST",body:"{}"},authFetch as typeof fetch);
      if(!result.seatToken||!result.franchiseId)throw new Error("The rejoin response was incomplete.");
      const next={roomCode:snapshot.room.code,hostToken:account.role==="commissioner"?credentials.hostToken:undefined,seatToken:result.seatToken,franchiseId:result.franchiseId,displayName:result.displayName};
      saveCredentials(next);if(result.snapshot)setSnapshot(result.snapshot);
      setMessage(`${account.franchiseName} has left autodraft. You control the next available turn.`);
      await announceChange();
    }catch(error){setMessage(error instanceof Error?error.message:"Could not resume manual drafting.");}
    finally{setBusy(false);}
  }
  ...
  async function hostAction(action: "start" | "pause" | "resume" | "undo") {
    ...
    try {
      const result = await requestJson(`/api/live-draft/rooms/${snapshot.room.code}/action`, {method: "POST", body: JSON.stringify({action, hostToken: credentials.hostToken})},authFetch as typeof fetch);
  ...
  async function draftPlayer(player: DraftPlayer) {
    ...
    try {
      const result = await requestJson(`/api/live-draft/rooms/${snapshot.room.code}/pick`, {method: "POST", body: JSON.stringify({actorToken: credentials.seatToken, player, expectedOverall: snapshot.room.currentOverall})},authFetch as typeof fetch);
  ...
  async function makeBotPick() {
    if (!snapshot || !credentials.hostToken) return; setBusy(true);
    try {
      const result = await requestJson(`/api/live-draft/rooms/${snapshot.room.code}/ai-pick`, {method: "POST", body: JSON.stringify({hostToken: credentials.hostToken})},authFetch as typeof fetch);
  ...
  useEffect(()=>{
    if(authLoading||!account||account.mustChangePassword||!snapshot||snapshot.room.status!=="lobby")return;
    if(credentials.franchiseId===account.franchiseId&&credentials.seatToken)return;
    const claimKey=`${snapshot.room.code}:${account.userId}`;
    if(joiningAccountRef.current===claimKey)return;
    joiningAccountRef.current=claimKey;setMessage(`Claiming ${account.franchiseName} for ${account.displayName}…`);
    void requestJson(`/api/live-draft/rooms/${snapshot.room.code}/join`,{method:"POST",body:"{}"},authFetch as typeof fetch).then(async(result)=>{
      if(!result.seatToken||!result.franchiseId)throw new Error("The authenticated seat response was incomplete.");
      const next={roomCode:credentials.roomCode,hostToken:account.role==="commissioner"?credentials.hostToken:undefined,seatToken:result.seatToken,franchiseId:result.franchiseId,displayName:result.displayName};
      saveCredentials(next);if(result.snapshot)setSnapshot(result.snapshot);setMessage(`${account.franchiseName} is claimed automatically by ${account.displayName}.`);await announceChange();
    }).catch((error)=>{joiningAccountRef.current="";setMessage(error instanceof Error?error.message:"Could not claim your authenticated franchise.")});
  },[account,authFetch,authLoading,credentials,snapshot]);
  ...
  const mySeat=snapshot?.seats.find((seat)=>seat.franchiseId===account?.franchiseId);
  const canPick = Boolean(snapshot?.room.status === "live" && account?.franchiseId===credentials.franchiseId && currentSeat?.franchiseId === credentials.franchiseId && !currentSeat?.autoDraft && credentials.seatToken);
  const isHost = Boolean(account?.role==="commissioner"&&credentials.hostToken);
  ...
  useEffect(() => {
    if (!snapshot || snapshot.room.status !== "live" || !currentSeat || (currentSeat.claimed&&!currentSeat.autoDraft) || automatedOverallRef.current === snapshot.room.currentOverall) return;
    const overall = snapshot.room.currentOverall;
    automatedOverallRef.current = overall;
    const timer = window.setTimeout(() => {
      void requestJson(`/api/live-draft/rooms/${snapshot.room.code}/ai-pick`, {method: "POST", body: "{}"},authFetch as typeof fetch)
        .then(async (result) => { if (result.snapshot) setSnapshot(result.snapshot); await announceChange(); })
  ...
  if (!snapshot) return <section className="liveDraftLaunch">
    <div className="liveLaunchIntro"><span className="liveTag">Account-secured multiplayer</span><h2>One board. Ten verified franchises.</h2><p>A Commissioner can load the exact projected keepers from the local Mock Draft or use the locked official board. Every AI seat now follows the same franchise personality and live PPR market as the one-person mock.</p><div className="liveFeatureStrip"><span>10 live seats</span><span>Same AI personalities</span><span>Persistent autodraft</span><span>Commissioner override</span></div></div>
    <div className="liveLaunchForms">
      {account?.role==="commissioner"&&!account.mustChangePassword?<form onSubmit={createRoom}><span>Commissioner · {account.displayName}</span><h3>Create a live room</h3><label>Room name<input value={roomName} onChange={(event) => setRoomName(event.target.value)} /></label><fieldset className="keeperSourceSetup"><legend>Keeper board</legend><label className={keeperSource==="projected"?"active":""}><input type="radio" name="keeperSource" value="projected" checked={keeperSource==="projected"} onChange={()=>setKeeperSource("projected")}/><span><b>Projected Mock Draft keepers</b><small>Loads the exact same 30 players and round costs as the local mock.</small></span></label><label className={keeperSource==="official"?"active":""}><input type="radio" name="keeperSource" value="official" checked={keeperSource==="official"} onChange={()=>setKeeperSource("official")}/><span><b>Locked official keepers</b><small>Requires the Commissioner keeper board to be finalized and locked.</small></span></label></fieldset><fieldset className="clockSetup"><legend>Seconds per pick</legend><div>{[10,30,60].map((value) => <button type="button" key={value} className={clockSeconds === value ? "active" : ""} onClick={() => setClockSeconds(value)}>{value === 60 ? "1 min" : `${value}s`}</button>)}<label>Custom<input aria-label="Custom seconds per pick" required type="number" min="1" max="3600" value={clockSeconds} onChange={(event) => setClockSeconds(Math.max(1, Math.min(3600, Number(event.target.value) || 1)))} /><span>sec</span></label></div></fieldset><button disabled={busy}>{keeperSource==="projected"?"Create room with projected keepers":"Create room with official keepers"}</button></form>:<div className="liveAuthNotice"><span>Commissioner</span><h3>Live room creation</h3><p>{authLoading?"Checking account access…":"A verified Commissioner account is required to create the live room."}</p>{!account&&<Link href="/login?next=/live-draft">Sign in</Link>}{account&&account.role!=="commissioner"&&<small>Signed in as {account.displayName} · {account.franchiseName}</small>}</div>}
  ...
  return <div className="draftRoomV3 liveDraftRoom">
    <section className="liveRoomTopbar"><div><span className="eyebrow">OKFL Live Draft · {snapshot.room.clockSeconds}s clock · {snapshot.room.keeperSource==="projected"?"Projected keepers":"Official keepers"}</span><h2>{snapshot.room.name}</h2><p>{message}</p></div><div className="roomCode"><span>Invite code</span><b>{snapshot.room.code}</b><button onClick={() => navigator.clipboard.writeText(`${window.location.origin}/live-draft?room=${snapshot.room.code}`)}>Copy link</button></div><div className="livePresence"><b>{onlineFranchises.size}</b><span>managers online</span></div></section>
    ...
    <section className="liveSeatLobby"><header><div><span className="eyebrow">Draft lobby</span><h2>{snapshot.seats.filter((seat) => seat.claimed).length}/10 franchises claimed</h2></div>{isHost && <div className="liveHostControls">
      ...
      {snapshot.room.status === "live" && <><button onClick={makeBotPick} disabled={busy}>Force AI pick</button><button onClick={() => hostAction("pause")} disabled={busy}>Pause</button></>}
      ...
      const aiControlled = snapshot.room.status !== "lobby" && (!seat.claimed||seat.autoDraft);
      return <article key={seat.franchiseId} className={`${seat.claimed ? "claimed" : ""} ${mine ? "mine" : ""} ${aiControlled ? "ai" : ""}`}><i>{aiControlled ? "AI" : seat.managerName.slice(0, 1)}</i><span>Pick {seat.slot}</span><b>{seat.managerName}</b><small>{seat.autoDraft?"Autodraft until rejoined":seat.claimedName || (aiControlled ? "AI manager" : "Awaiting manager")}</small><em>{seat.autoDraft?"Autodraft":onlineFranchises.has(seat.franchiseId) ? "Online" : seat.claimed ? "Authenticated · offline" : aiControlled ? "AI controlled" : "Waiting for sign-in"}</em></article>;
    })}</div>
      {snapshot.room.status === "lobby" && <div className="liveAuthenticatedSeat">{account?<><span>Authenticated franchise</span><b>{account.franchiseName} · {account.displayName}</b><small>{credentials.seatToken&&credentials.franchiseId===account.franchiseId?"Seat claimed. You are ready to draft.":"Your account is claiming its assigned seat automatically."}</small></>:<><span>Manager seat required</span><b>Sign in to claim your franchise</b><Link href={`/login?next=/live-draft?room=${snapshot.room.code}`}>Sign in</Link></>}</div>}
      {snapshot.room.status !== "lobby"&&mySeat?.autoDraft&&<div className="liveAuthenticatedSeat autoDraftReturn"><span>Autodraft active</span><b>{account?.franchiseName} missed the clock</b><small>The same franchise AI will keep selecting until you return.</small><button onClick={rejoinDraft} disabled={busy}>{busy?"Rejoining…":"Resume manual drafting"}</button></div>}
    </section>

    <section className="draftV2BoardPanel liveBoard"><header><div><span className="eyebrow">Room {snapshot.room.code} · {snapshot.room.status}</span><h2>Live draft board</h2></div><div className={`clockStatus liveClock ${canPick ? "yourTurn" : ""} ${secondsRemaining <= 5 ? "urgent" : ""}`}><span>{canPick ? "Your pick" : currentSeat?.autoDraft?`${currentSeat.managerName} AI`:currentSeat?.claimedName || (currentSeat ? "AI selecting" : "Draft complete")}</span><b>{snapshot.room.status === "live" ? secondsRemaining : "—"}<small>{snapshot.room.status === "live" ? " sec" : current ? `${current.round}.${current.slot}` : "Final"}</small></b></div></header>
    ...
    <div className="liveDraftWorkbench"><section className="livePlayerPool"><header><div><span className="eyebrow">PPR player market</span><h2>{canPick ? "You are on the clock" : "Available players"}</h2></div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search players" /></header><div className="positionTabs">{["ALL","QB","RB","WR","TE","K","DEF"].map((value) => <button key={value} className={position === value ? "active" : ""} onClick={() => setPosition(value)}>{value}</button>)}</div><div className="livePlayerRows">{available.slice(0, 180).map((player) => <article key={player.name}><span className={`playerPosition ${POSITION_CLASS[player.position] || ""}`}>{player.position}</span><div><b>{player.name}</b><small>{player.team} · ADP {player.marketAdp?.toFixed(1) || player.pprRank}</small></div><strong>#{player.pprRank}</strong><button disabled={!canPick || busy} onClick={() => draftPlayer(player)}>Draft</button></article>)}</div></section>
      <aside className="liveDraftSide"><section><span className="eyebrow">On the clock · {secondsRemaining}s</span><h2>{currentSeat?.autoDraft?`${currentSeat.managerName} AI`:currentSeat?.claimedName || (currentSeat ? `${currentSeat.managerName} AI` : "Draft complete")}</h2><p>{snapshot.room.status === "lobby" ? "Waiting for the commissioner to start. Unclaimed teams become AI." : canPick ? "Choose a player before the clock expires." : currentSeat?.autoDraft?"This franchise remains on autodraft until its manager rejoins.":currentSeat?.claimed ? "Waiting for this manager's selection. Missing the clock activates persistent autodraft." : "AI is making this franchise's selection."}</p>{isHost && snapshot.room.status === "live" && <button onClick={makeBotPick} disabled={busy}>Force AI pick now</button>}</section><section><span className="eyebrow">My roster</span><h3>{managers.find((manager) => manager.franchiseId === credentials.franchiseId)?.manager || "Spectator"}</h3><div className="liveRoster">{myRoster.map((pick) => <div key={pick.overall}><span>{pick.player.position}</span><b>{pick.player.name}</b><small>{pick.keeper ? `Keeper R${pick.keeperCost}` : `${pick.round}.${pick.slot}`}</small></div>)}{!myRoster.length && <p>Claim a franchise to build your roster.</p>}</div></section></aside>
    </div>
  </div>;
}
