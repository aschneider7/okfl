...
export function DraftHeader() {
  const {started, controlledManager, lastMessage, overall, complete, paused, isSimulating, draftMode, rankingsMeta, rankingsLoading} = useDraft();
  const updated = rankingsMeta.updatedAt ? new Date(rankingsMeta.updatedAt).toLocaleDateString("en-US", {month: "short", day: "numeric", timeZone: "UTC"}) : "offline";
  const sample = rankingsMeta.totalDrafts ? `${rankingsMeta.totalDrafts.toLocaleString()} mocks · ` : "";
  return <>
    <section className="draftV2Setup">
      <div className="draftV2Brand"><span className="eyebrow">OKFL Draft Room <i>V4.5.0</i></span>
