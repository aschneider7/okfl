...
export async function GET(request:Request){
  try{
    ...
    const [communicationsResult,votesResult,eligibleResult]=await Promise.all([supabase.from("league_communications").select("id,kind,title,body,choices,status,closes_at,created_at,closed_at").in("id",ids).order("created_at",{ascending:false}),supabase.from("league_votes").select("communication_id,user_id,option_id").in("communication_id",ids),supabase.from("league_communication_recipients").select("communication_id").in("communication_id",ids)]);
    if(communicationsResult.error||votesResult.error||eligibleResult.error)throw communicationsResult.error||votesResult.error||eligibleResult.error;
    const now=Date.now(),votes=votesResult.data||[],eligible=eligibleResult.data||[];
    const communications=(communicationsResult.data||[]).map((row:any)=>{const rowVotes=votes.filter((vote:any)=>vote.communication_id===row.id);const expired=Boolean(row.closes_at&&new Date(row.closes_at).getTime()<=now);return {...row,isOpen:row.kind==="poll"&&row.status==="open"&&!expired,myVote:rowVotes.find((vote:any)=>vote.user_id===account.userId)?.option_id||null,totalVotes:rowVotes.length,eligibleCount:eligible.filter((recipient:any)=>recipient.communication_id===row.id).length,results:(Array.isArray(row.choices)?row.choices:[]).map((choice:any)=>({...choice,votes:rowVotes.filter((vote:any)=>vote.option_id===choice.id).length}))};});
