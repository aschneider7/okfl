...
export async function POST(request:Request){
  try{
    ...
    const keeperSource=body.keeperSource==="projected"?"projected":"official";
    const keepers=keeperSource==="projected"?projectedKeepers:await getLockedKeeperBoard();
    if(!keepers)return NextResponse.json({error:"Lock the final official keeper board in Commissioner OS, or create the room with projected Mock Draft keepers."},{status:409});
    ...
    const keeperRows=keepers.map((keeper)=>{const manager=managers.find((row)=>row.franchiseId===keeper.franchiseId)!;const overall=keeperOverall(keeper.round,manager.slot);return{
      room_id:room!.id,overall,round:keeper.round,slot:manager.slot,franchise_id:keeper.franchiseId,player_key:draftPlayerKey(keeper.player),keeper:true,keeper_cost:keeper.round,selected_by:keeperSource==="projected"?"projected mock keeper":"official keeper",
      player:{name:keeper.player,position:keeper.position,team:"—",pprRank:999,pprValue:0,keeperEligible:false,source:keeperSource==="projected"?"projected-keeper":"official-keeper"}}});
