...
export async function POST(request: Request, context: {params: Promise<{code: string}>}) {
  try {
    ...
    const {data: room} = await supabase.from("live_draft_rooms").select("id,status,host_token_hash").eq("code", normalized).maybeSingle();
    ...
    if (action === "start") {
      ...
      const {error} = await supabase.from("live_draft_rooms").update({status: firstOpen > 170 ? "complete" : "live", current_overall: Math.min(firstOpen, 171), updated_at: new Date().toISOString()}).eq("id", room.id);
      if (error) throw error;
    } else if (action === "pause" || action === "resume") {
      const status = action === "pause" ? "paused" : "live";
      const {error} = await supabase.from("live_draft_rooms").update({status, updated_at: new Date().toISOString()}).eq("id", room.id);
      if (error) throw error;
    } else if (action === "undo") {
      ...
      const {error: roomError} = await supabase.from("live_draft_rooms").update({status: "paused", current_overall: lastPick.overall, updated_at: new Date().toISOString()}).eq("id", room.id);
