...
for(const token of ["getAccountFromRequest","communication_id,user_id","Voting is closed","optionId"]){assert.ok(managerRoute.includes(token),`Manager ballot API is missing ${token}`)}
...
for(const token of ["one-franchise-one-vote","Your franchise has voted","vote("]){assert.ok(ballot.includes(token),`League Votes UI is missing ${token}`)}
