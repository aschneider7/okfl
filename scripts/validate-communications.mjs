...
for(const token of ["Commissioner account required","manager_notifications","sendLeagueSms","not_consented","recipientUserIds","voters","optionLabel","updated_at"]){assert.ok(commissionerRoute.includes(token),`Commissioner communications API is missing ${token}`)}
...
for(const token of ["Phone directory","Manager consented to SMS","League vote","Delivery & voting history","Named vote ledger","Has not voted","Commissioner only"]){assert.ok(commissioner.includes(token),`Commissioner UI is missing ${token}`)}
